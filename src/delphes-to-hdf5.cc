...
// Throws if multiple top or anti-top quarks are found or if one of them is
// missing
...
findTT(const std::vector<const GenParticle*>& particle_vec)
{
  ...
  if ((t0 == nullptr) or (t1 == nullptr)) {
    throw std::runtime_error(
      "failed to find top and anti-top quarks in the event");
  }
  return { t0, t1 };
}
                 ...
                 const std::vector<const GenParticle*>& p_vec)
{
  ...
  if (daughters.size() != 2) {
    throw std::runtime_error(
      "expected exactly two daughters for particle PID " +
...
// Returns two daughters of W: (lepton, neutrino) or (down-type quark, up-type
// quark) Both daughters are first copies
std::pair<const GenParticle*, const GenParticle*>
    ...
    const bool enable_progress_bar)
{
  ...
  const size_t truth_particles_size =
    10; // t, b, W, l, nu for both top and anti-top
  ...
  auto met_dataset =
    output_group.createDataSet<METData>("MET", met_space, met_props);
  ...
  decay_channel_props.add(
    Chunking{ std::vector<hsize_t>{ output_chunk_size } });
  auto decay_channel_dataset = output_group.createDataSet<int16_t>(
    "decay_channel", decay_channel_space, decay_channel_props);

  // jets
  spdlog::info("  - Creating jets dataset...");
  DataSpace jets_space({ 0, max_jets_size },
                       { DataSpace::UNLIMITED, max_jets_size });
  DataSetCreateProps jets_props{};
  jets_props.add(
    Chunking{ std::vector<hsize_t>{ output_chunk_size, max_jets_size } });
  auto jets_dataset =
    output_group.createDataSet<JetData>("jets", jets_space, jets_props);

  // jets_indices
  spdlog::info("  - Creating jets_indices dataset...");
  DataSpace jets_indices_space({ 0, max_jets_size },
                               { DataSpace::UNLIMITED, max_jets_size });
  DataSetCreateProps jets_indices_props{};
  jets_indices_props.add(
    Chunking{ std::vector<hsize_t>{ output_chunk_size, max_jets_size } });
  auto jets_indices_dataset = output_group.createDataSet<int16_t>(
    "jets_indices", jets_indices_space, jets_indices_props);

  // leptons
  spdlog::info("  - Creating leptons dataset...");
  DataSpace leptons_space({ 0, lepton_size },
                          { DataSpace::UNLIMITED, lepton_size });
  DataSetCreateProps leptons_props;
  leptons_props.add(
    Chunking{ std::vector<hsize_t>{ output_chunk_size, lepton_size } });
  auto leptons_dataset = output_group.createDataSet<LeptonData>(
    "leptons", leptons_space, leptons_props);
  ...
  auto matchability_dataset = output_group.createDataSet<int16_t>(
    "matchability", matchability_space, matchability_props);
  ...
  auto nbjets_dataset =
    output_group.createDataSet<int16_t>("nbjets", nbjets_space, nbjets_props);

  // neutrinos
  spdlog::info("  - Creating neutrinos dataset...");
  DataSpace neutrinos_space({ 0, neutrino_size },
                            { DataSpace::UNLIMITED, neutrino_size });
  DataSetCreateProps neutrinos_props{};
  neutrinos_props.add(
    Chunking{ std::vector<hsize_t>{ output_chunk_size, neutrino_size } });
  auto neutrinos_dataset = output_group.createDataSet<ParticleData>(
    "neutrinos", neutrinos_space, neutrinos_props);
  ...
  auto njets_dataset =
    output_group.createDataSet<int16_t>("njets", njets_space, njets_props);

  // truth_particles
  spdlog::info("  - Creating truth_particles dataset...");
  DataSpace truth_particles_space(
    { 0, truth_particles_size },
    { DataSpace::UNLIMITED, truth_particles_size });
  DataSetCreateProps truth_particles_props{};
  truth_particles_props.add(Chunking{
    std::vector<hsize_t>{ output_chunk_size, truth_particles_size } });
  auto truth_particles_dataset = output_group.createDataSet<ParticleData>(
    "truth_particles", truth_particles_space, truth_particles_props);

  // truth_quarks
  spdlog::info("  - Creating truth_quarks dataset...");
  DataSpace truth_quarks_space({ 0, truth_quarks_size },
                               { DataSpace::UNLIMITED, truth_quarks_size });
  DataSetCreateProps truth_quarks_props{};
  truth_quarks_props.add(
    Chunking{ std::vector<hsize_t>{ output_chunk_size, truth_quarks_size } });
  auto truth_quarks_dataset = output_group.createDataSet<ParticleData>(
    "truth_quarks", truth_quarks_space, truth_quarks_props);
  ...
  const uint64_t jet_btag_bit =
    0; // NOTE:
       // https://github.com/delphes/delphes/blob/master/cards/delphes_card_CMS.tcl#L720
  ...
  spdlog::info(
    "  - jet pT > {} GeV and |eta| < {}", jet_pt_min, jet_abs_eta_max);
  spdlog::info("  - electron pT > {} GeV and |eta| < {}",
               electron_pt_min,
               electron_abs_eta_max);
  spdlog::info(
    "  - muon pT > {} GeV and |eta| < {}", muon_pt_min, muon_abs_eta_max);
  spdlog::info("  - parton-jet matching max deltaR = {}",
               parton_jet_matching_max_dr);
  ...
  const auto entry_stop =
    (max_events > 0) ? std::min(static_cast<size_t>(max_events), total) : total;
  ...
  for (size_t entry = 0; entry < entry_stop; ++entry) {
    ...
    const size_t jet_stop = std::min(
      static_cast<size_t>(input_jet_array->GetEntries()), max_jets_size);
    for (size_t jet_idx = 0; jet_idx < jet_stop; ++jet_idx) {
      if (const auto jet =
            dynamic_cast<const Jet*>(input_jet_array->At(jet_idx))) {
    ...
    for (int idx = 0; idx < input_electron_array->GetEntries(); ++idx) {
      if (const auto electron =
            dynamic_cast<const Electron*>(input_electron_array->At(idx))) {
        if ((electron->PT > electron_pt_min) and
            (std::abs(electron->Eta) < electron_abs_eta_max)) {
    ...
    for (int idx = 0; idx < input_muon_array->GetEntries(); ++idx) {
      if (const auto muon =
            dynamic_cast<const Muon*>(input_muon_array->At(idx))) {
        if ((muon->PT > muon_pt_min) and
            (std::abs(muon->Eta) < muon_abs_eta_max)) {
    ...
    for (int idx = 0; idx < input_particle_array->GetEntries(); ++idx) {
      const auto particle =
        dynamic_cast<const GenParticle*>(input_particle_array->At(idx));
    ...
    const auto parton_jet_idx_vec = matchPartonToJet(
      parton_vec, jet_vec, /*max_distance=*/parton_jet_matching_max_dr);
    ...
    const int16_t decay_channel_data =
      static_cast<int16_t>(decay_channel_bits.to_ulong());
    ...
    for (const auto& lepton : lepton_vec) {
      std::visit(
        [&leptons_data](auto&& arg) {
          using T = std::decay_t<decltype(arg)>;

          if constexpr (std::is_same_v<T, const Electron*>) {
            leptons_data.emplace_back(arg->PT,
                                      arg->Eta,
                                      arg->Phi,
    ...
    for (size_t parton_idx = 0; parton_idx < parton_vec.size(); ++parton_idx) {
      matchability_bits.set(parton_idx,
                            parton_jet_idx_vec.at(parton_idx) >= 0); // b
    }
    const int16_t matchability_data =
      static_cast<int16_t>(matchability_bits.to_ulong());

    // NOTE: nbjets
    const int16_t nbjets_data = static_cast<int16_t>(
      std::count_if(jet_vec.cbegin(), jet_vec.cend(), [](const Jet* jet) {
        return checkBit(jet->BTag, jet_btag_bit);
      }));
    ...
    for (const auto& nu : { nu_0, nu_1 }) {
      neutrinos_data.emplace_back(static_cast<int32_t>(nu->PID),
                                  nu->PT,
                                  nu->Eta,
                                  nu->Phi,
    ...
    const auto truth_particle_vec = {
      t0, b0, w0, lep_0, nu_0, t1, b1, w1, lep_1, nu_1,
    };

    for (const auto& particle : truth_particle_vec) {
      truth_particles_data.emplace_back(static_cast<int32_t>(particle->PID),
                                        particle->PT,
                                        particle->Eta,
                                        particle->Phi,
    ...
    for (const auto& quark : { b0, b1 }) {
      truth_quarks_data.emplace_back(static_cast<int32_t>(quark->PID),
                                     quark->PT,
                                     quark->Eta,
                                     quark->Phi,
    ...
    decay_channel_dataset.select({ output_idx }, { 1 })
      .write(&decay_channel_data);
    jets_dataset.select({ output_idx, 0 }, { 1, max_jets_size })
      .write_raw(jets_data.data());
    jets_indices_dataset.select({ output_idx, 0 }, { 1, max_jets_size })
      .write_raw(jets_indices_data.data());
    leptons_dataset.select({ output_idx, 0 }, { 1, lepton_size })
      .write_raw(leptons_data.data());
    matchability_dataset.select({ output_idx }, { 1 })
      .write(&matchability_data);
    nbjets_dataset.select({ output_idx }, { 1 }).write(&nbjets_data);
    neutrinos_dataset.select({ output_idx, 0 }, { 1, neutrino_size })
      .write_raw(neutrinos_data.data());
    njets_dataset.select({ output_idx }, { 1 }).write(&njets_data);
    truth_particles_dataset
      .select({ output_idx, 0 }, { 1, truth_particles_size })
      .write_raw(truth_particles_data.data());
    truth_quarks_dataset.select({ output_idx, 0 }, { 1, truth_quarks_size })
      .write_raw(truth_quarks_data.data());
  }
  ...
  spdlog::info("  - Unweighted selection efficiency: {:.2f}%",
               100 * unweighted_efficiency);
}
...
main(int argc, char* argv[])
{
  ...
  spdlog::info("  - progress bar: {}",
               enable_progress_bar ? "enabled" : "disabled");
  ...
  try {
    ...
    run(input_path, output_path, max_events, enable_progress_bar);
